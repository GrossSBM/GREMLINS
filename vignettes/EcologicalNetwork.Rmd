...
**A multipartite network is a collection of networks**:  each network may be *simple* if it represents the relations inside  a   functional   group  or *bipartite* if it represent the relations between individuals of two  functional groups. We index the collection of networks by pairs of functional groups $(q,q')$. 
...
Note that,  if $q \neq q'$,   $X^{qq'}$ is  said to be  an **incidence matrix** (corresponding to a bipartite network). If $q = q'$, $X^{qq}$ is  an  **adjacency matrix**:   if the relation inside the functional group $q$  is non-oriented, $X^{qq}$ is symmetric. 
...
The random variables $Z^{q}_i$'s are assumed to be independent and  such that: $\forall (k,q,i)$:
...
## Example of an ecological network

### The dataset
We apply our methodology to an ecological mutualistic multipartite network. The data were  provided in @Dattilo.

In this example, the functional groups are plants, floral visitors, ants and birds and one observes binary interactions between 1/ plants and florals visitor,  2/ plants and ants, and 3/ plants and seed dispersal birds, resulting into three bipartite networks.   
...
As required by GREMLIN, our mutlipartite Network  the global network  to be be encoded in separate matrices for each network (in our case the $3$ incidence matrices)
...
### Formatting the data
We format the data to be able to use our R package GREMLIN i.e. we transform the matrices into an list containing    

              * the matrix, 
              * its type  
                           - `inc`    : incidence 
                           -  `adj`   : adjacency symetric 
                           - `dir_adj`: adjacency non symetric  
              * the name of functional group in row, 
              * the name of functional group in column. 
...
## An example on a simulated dataset
...
A,B and C are  divided respectively into $4$, $3$ and $2$ blocks. The sizes of the blocks are generated randomly. For reproductibility, we set the random seed to an arbitrarily chosen value.  
